...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                print(f"Proto type: {type(proto[i])}")  # Should be torch.Tensor, not list
                print(f"Detections shape: {det.shape}")  # Check shape of detections
            ...
            # --- New MJPEG streaming output ---
            if view_img:  # We'll reuse 'view_img' flag to control streaming mode
                ...
                # Yield frame in MJPEG HTTP format
                yield (b'--frame\r\n'
